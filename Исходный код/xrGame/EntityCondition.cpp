...
// Предел максимального здоровья теперь считывается из конфига
#define MAX_HEALTH 1.0f
#define MAX_HEALTH_LIMIT 10.0f
...
:CEntityConditionSimple()
{
	...
	// Lex Addon (correct by Suhar_) 25.05.2018		(begin)
	// Предел максимального здоровья теперь считывается из конфига
	m_fMaxHealthLimit		= MAX_HEALTH_LIMIT;
	// Lex Addon (correct by Suhar_) 25.05.2018		(end)
...
void CEntityCondition::LoadCondition(LPCSTR entity_section)
{
	...
	// Lex Addon (correct by Suhar_) 25.05.2018		(begin)
	// Предел максимального здоровья теперь считывается из конфига
	m_fMaxHealthLimit		= READ_IF_EXISTS(pSettings,r_float,entity_section,"MaxHealthValue",MAX_HEALTH*100)/100;
	// Lex Addon (correct by Suhar_) 25.05.2018		(end)
...
void CEntityCondition::UpdateCondition()
{
	...
	// Lex Addon (correct by Suhar_) 25.05.2018		(begin)
	// Предел максимального здоровья теперь считывается из конфига
		// Изначально при инициализации в max_health() записываются большие значения MAX_HEALTH_LIMIT
		// (пока не собраны данные, с других апдейтов о добавках к максимальному уровню здоровья, например с костюмов и артефактов)
		// что позволяет фильтру не обрубать только что загруженное состояние здоровья (если оно выходит за дефолтный максимум).
		// В это время в m_fMaxHealthLimit уже записано считанное из конфига значение предела уровня здоровья,
		// что исключает возможность перебора HP у маложивучей живности (главное, отрегулировать это в конфигах)
		// При дальнейших апдейтах в max_health() уже будет записано правильное значение предела.
	clamp						(l_health,			MIN_HEALTH, max_health());
	clamp						(l_health,			MIN_HEALTH, m_fMaxHealthLimit);
	// Lex Addon (correct by Suhar_) 25.05.2018		(end)
	SetHealth					(l_health);
...
// Функция вычисления повреждений
CWound* CEntityCondition::ConditionHit(SHit* pHDS)
{
	...
	// Учитываем характеристики брони (костюм+шлем)
