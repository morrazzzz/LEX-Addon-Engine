...
void CUIInventoryUpgradeWnd::LoadSchemes( CUIXml& uiXml )
{
	...
	Frect	t_cell_item;
	// Lex Addon (correct by Suhar_) 14.09.2016		(begin)
	// edit 15.09.2016
	// Дополнен алгоритм чтения параметров иконки апгрейда
	...
	for ( int i_tmpl = 0; i_tmpl < tmpl_count; ++i_tmpl )
	{
		...
		for ( int i_clm = 0; i_clm < clm_count; ++i_clm )
		{
			...
			{
				d_cell_item.x1 = t_cell_item.x1;
				d_cell_item.y1 = t_cell_item.y1;
				d_cell_item.x2 = d_cell_item.x1 + uiXml.ReadAttribFlt("cell", i_cell, "width", t_cell_item.x2)*(UI().is_widescreen()?0.8f:1.0f);
				d_cell_item.y2 = d_cell_item.y1 + uiXml.ReadAttribFlt("cell", i_cell, "height", t_cell_item.y2);
				UIUpgrade* item = xr_new<UIUpgrade>( this );
				item->load_from_xml( uiXml, i_clm, i_cell, d_cell_item );
	// Lex Addon (correct by Suhar_) 14.09.2016		(end)
