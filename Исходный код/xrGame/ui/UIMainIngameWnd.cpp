...
void CUIMainIngameWnd::Draw()
{
	...
	// Добавлена возможность скрывать с худа мини-карту по наличию infoporion (в одиночной игре)
	// Инвентарь закрыт
	if ( !IsGameTypeSingle() || ( IsGameTypeSingle() && !Actor()->HasInfo("blocked_hud_minimap_info") ) )
	{
		UIMotionIcon->Draw();
...
// Lex Addon (correct by Suhar_) 5.11.2015 + 28.09.2016 + 25.02.2017		(begin)
void CUIMainIngameWnd::UpdateMainIndicators()
{
	// 25.02.2017
	// Инвентарь закрыт
	// Добавлена возможность скрывать с худа иконки индикаторов по наличию infoporion (в одиночной игре)
	bool hide_hud_indicators = IsGameTypeSingle() && Actor()->HasInfo("blocked_hud_indicators_info");
	CActor* pActor = smart_cast<CActor*>(Level().CurrentViewEntity());
	if(!pActor)
		return;
	...
	if (fis_zero(bleeding, EPS) || hide_hud_indicators)
	{
	...
	{
		// 5.11.2015
		// Дополнен алгоритм вывода на экран индикатора кровотечения 
	...
	if(fis_zero(radiation, EPS) || hide_hud_indicators)
	{
	...
	{
		// 5.11.2015
		// Дополнен алгоритм вывода на экран индикатора облучённости
	...
	// 5.11.2015
	// Дополнены алгоритмы вывода на экран индикаторов состояния предметов в слоте (оружие, шлем, костюм), голода, перегруза
	// Satiety icon
	// UPD: 20.10.2018
	float satiety = pActor->conditions().GetSatiety();
	float satiety_critical = pActor->conditions().SatietyCritical();
	float satiety_extra_low = satiety_critical + (1 - satiety_critical) / 4;
	float satiety_low = satiety_extra_low + (1 - satiety_critical) / 4;
	float satiety_average = satiety_low + (1 - satiety_critical) / 4;
	if (satiety>satiety_average || hide_hud_indicators)
	...
	else
	{
		...
		else if (satiety<=satiety_extra_low)
			m_ind_starvation->InitTexture("ui_inGame2_circle_hunger_orange");
		else if (satiety<=satiety_low)
	...
	// 28.09.2016
	// Распространяем проверку изношенности на предмет в слоте ножа
	if (slot==INV_SLOT_2 || slot==INV_SLOT_3 || slot==KNIFE_SLOT)
	{
		CWeapon* weapon = smart_cast<CWeapon*>(pActor->inventory().ItemFromSlot(slot));
		if (weapon && !hide_hud_indicators)
		{
			...
			if(condition<start_misf_cond)
			{
				...
				if(condition>(start_misf_cond+start_misf_cond+end_misf_cond)/3)
					m_ind_weapon_broken->InitTexture("ui_inGame2_circle_Gunbroken_green");
				else if(condition>(start_misf_cond+end_misf_cond+end_misf_cond)/3)
	...
	// UPD: 20.10.2018
	if (cur_weight>=max_weight-15.0f && IsGameTypeSingle() && !hide_hud_indicators)
	{
...
// Lex Addon (correct by Suhar_) 5.11.2015 + 28.09.2016 + 25.02.2017		(end)

// Lex Addon (correct by Suhar_) 25.02.2017		(begin)
// Добавлена возможность скрывать с худа иконки слотов быстрого использования по наличию infoporion (в одиночной игре)
void CUIMainIngameWnd::UpdateQuickSlots()
{
	bool hide_hud_quick_slots = IsGameTypeSingle() && Actor()->HasInfo("blocked_hud_quick_slots_info");
	string32 tmp;
	LPCSTR str = CStringTable().translate("quick_use_str_1").c_str();
	strncpy_s(tmp, sizeof(tmp), str, 3);
	if(tmp[2]==',')
		tmp[1] = '\0';
	if ( hide_hud_quick_slots )
	...
	for(u8 i=0;i<4;i++)
	{
		...
		if(wnd)
		{
			...
			if ( item_name.size() && !hide_hud_quick_slots )
			{
				...
				m_quick_slots_icons[i]->Show(true);
...
void CUIMainIngameWnd::DrawMainIndicatorsForInventory()
{
	...
	// Открыт инвентарь
	// Добавлена возможность скрывать с худа иконки слотов быстрого использования по наличию infoporion (в одиночной игре)
	if ( !IsGameTypeSingle() || ( IsGameTypeSingle() && !Actor()->HasInfo("blocked_hud_quick_slots_info") ) )
	{
		for(int i=0;i<4;i++)
	...
	// Добавлена возможность скрывать с худа иконки индикаторов по наличию infoporion (в одиночной игре)
	if ( !IsGameTypeSingle() || ( IsGameTypeSingle() && !Actor()->HasInfo("blocked_hud_indicators_info") ) )
	{
...
void CUIMainIngameWnd::UpdateBoosterIndicators(const xr_map<EBoostParams, SBooster> influences)
{
	...
	// Инвентарь закрыт
	// Добавлена возможность скрывать с худа иконки индикаторов по наличию infoporion (в одиночной игре)
	if (IsGameTypeSingle() && Actor()->HasInfo("blocked_hud_indicators_info") )
		return;
	// Lex Addon (correct by Suhar_) 25.02.2017		(end)
	...
	for(; b!=e; b++)
	{
		switch(b->second.m_type)
		{
			...
			// Убраны иконки отображения eBoostRadiationImmunity, eBoostTelepaticImmunity, eBoostChemicalBurnImmunit
			// т.к. для корректного отображения работы некоторых бустеров необходим полный набор, в т.ч. eBoostBurnProtection и т.д.
