...
void ui_actor_state_wnd::UpdateActorInfo( CInventoryOwner* owner )
{
	...
	// Lex Addon (correct by Suhar_) 4.07.2016		(begin)
	// Учитываем при отображении индикатора здоровья ГГ максимальное значение
	value = (actor->conditions().GetHealth())/(actor->conditions().GetMaxHealth());
	// Lex Addon (correct by Suhar_) 4.07.2016		(end)
...
// show bleeding icon
	...
	// Lex Addon (correct by Suhar_) 6.10.2015		(begin)
	// Добавляем ещё один индикатор к отображению интенсивности кровотечения
	m_state[stt_bleeding]->show_static(false, 4);
	if(!fis_zero(value, EPS))
	{
		if(value<0.25f)
			...
			m_state[stt_bleeding]->show_static(true, 4);
	// Lex Addon (correct by Suhar_) 6.10.2015		(end)
	}
// show radiation icon
	...
	// Lex Addon (correct by Suhar_) 6.10.2015		(begin)
	// Добавляем ещё один индикатор к отображению уровня облучённости
	m_state[stt_radiation]->show_static(false, 4);
	if(!fis_zero(value, EPS))
	{
		if(value<0.25f)
			...
			m_state[stt_radiation]->show_static(true, 4);
	// Lex Addon (correct by Suhar_) 6.10.2015		(end)
	}
	...
	if(outfit)
	{
		...
		float outfit_condition = outfit->GetCondition() * 0.9 + 0.1;
		fwou_value += outfit->GetBoneArmor(spine_bone) * outfit_condition;					
	...
	if(helmet)
	{
		...
		// Изменение формулы учёта состояния у шлемов и костюмов
		// Шлемы и костюмы дают небольшую защиту даже при очень плохом состоянии (10% защиты от максимума)
		fwou_value += helmet->GetBoneArmor(spine_bone) * (helmet->GetCondition() * 0.9 + 0.1);
		// Lex Addon (correct by Suhar_) 7.12.2018		(end)
	}
	
// Lex Addon (correct by Suhar_)			(begin)
// Слегка меняем алгоритм отображения защиты от различных воздействий в инвентаре
//fire burn protection progress bar
	{
		...
		burn_value = (burn_value / max_power); // number of sticks in progress bar
...
//radiation protection progress bar
	{
		...
		radi_value = (radi_value / max_power); // number of sticks in progress bar
...
//chemical burn protection progress bar
	{
		...
		cmbn_value = (cmbn_value / max_power); // number of sticks in progress bar
...
//shock protection progress bar
	{
		...
		shoc_value = (shoc_value / max_power); // number of sticks in progress bar  
...
//fire wound protection progress bar
	{
		...
		fwou_value = (fwou_value / max_power); // number of sticks in progress bar
		m_state[stt_fire_wound]->set_progress(fwou_value);
	}
//power restore speed progress bar
	{
		value = actor->GetRestoreSpeed(ALife::ePowerRestoreSpeed) / actor->conditions().GetMaxPowerRestoreSpeed();; // number of sticks in progress bar  
...
ui_actor_state_item::ui_actor_state_item()
{
	...
	m_static2		= NULL;
	m_static3		= NULL;
	// Lex Addon (correct by Suhar_) 12.04.2016		(begin)
	// Описываем переменную дополнительного индикатора
	m_static4		= NULL;
	// Lex Addon (correct by Suhar_) 12.04.2016		(end)
	m_progress		= NULL;
	m_sensor		= NULL;
	m_arrow			= NULL;
	m_arrow_shadow	= NULL;
...
//		m_magnitude = xml.ReadAttribFlt( "icon", 0, "magnitude", 1.0f );
	...
	// Lex Addon (correct by Suhar_) 12.04.2016		(begin)
	// Считываем параметры текстуры из .xml для дополнительного индикатора
	if ( xml.NavigateToNode( "icon4", 0 ) )	
	{
		m_static4 = UIHelper::CreateStatic( xml, "icon4", this );
		m_static4->TextItemControl()->SetText("");
	}
	// Lex Addon (correct by Suhar_) 12.04.2016		(end)
	set_arrow( 0.0f );
	xml.SetLocalRoot( stored_root );
...
void ui_actor_state_item::show_static( bool status, u8 number )
{
	...
	case 3:
		...
		break;
	// Lex Addon (correct by Suhar_) 12.04.2016		(begin)
	// Добавляем ещё один индикатор
	case 4:
		if(!m_static4)
			return;
		m_static4->Show(status);
		break;
	// Lex Addon (correct by Suhar_) 12.04.2016		(end)
	default:
		break;
