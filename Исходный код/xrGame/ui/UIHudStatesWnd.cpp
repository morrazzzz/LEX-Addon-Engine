...
void CUIHudStatesWnd::Update()
{
	...
	// Добавлена возможность скрывать с худа правую нижнюю панель по наличию infoporion (в одиночной игре)
	if ( IsGameTypeSingle() && Actor()->HasInfo("blocked_hud_right_down_pannel_info") )
	{
		m_back->Show(false);
...
//	}
	...
	// Учитываем при отображении индикатора здоровья ГГ его максимальное значение
	m_ui_health_bar->SetProgressPos(iCeil(cur_health / (actor->conditions().GetMaxHealth()) * 100.0f * 35.f) / 35.f );
...
void CUIHudStatesWnd::UpdateIndicatorType(CActor* actor, ALife::EInfluenceType type)
{
	...
	// Добавляем оранжевый индикатор опасности
	// Меняем порядок отображения индикаторов
	int protect_k = 1;
	switch(type)
	{
		case ALife::infl_rad: texture = "ui_inGame2_triangle_Radiation_"; protect_k = 10; break;
		...
		case ALife::infl_psi: texture = "ui_inGame2_triangle_Psy_"; protect_k = 10; break;
		default: NODEFAULT;
	}
	...
	if (hit_power <= EPS || IsGameTypeSingle() && Actor()->HasInfo("blocked_hud_indicators_info"))
	// 25.02.2017		(end)
	{
	...
	// Оранжевый индиктор
	else if (hit_power - protect <= m_zone_threshold[type] * 2)
		xr_sprintf(str, sizeof(str), "%s%s", texture, "orange");
	// Красный индиктор
	else
