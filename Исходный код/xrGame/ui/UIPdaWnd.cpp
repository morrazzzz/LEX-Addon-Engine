...
// Lex Addon (correct by Suhar_) 15.08.2018		(begin)
// Возможность переключаться между разделами только при наличии инфопоршна
// #include "../actor.h"
// Lex Addon (correct by Suhar_) 15.08.2018		(end)
...
*/
	...
	// Дополнительная кнопка переключения разделов
	m_btn_switch			= UIHelper::Create3tButton( uiXml, "switch_button", this );
	...
	case BUTTON_CLICKED:
		{
			...
			// Lex Addon (correct by Suhar_) 10.04.2017		(begin)
			// Дополнительная кнопка переключения разделов
			else if ( pWnd == m_btn_switch )
			{
				luabind::functor<void> PdaActionSwitch;
				if (ai().script_engine().functor("_g.pda_action_switch", PdaActionSwitch))
					PdaActionSwitch();
			}
			// Lex Addon (correct by Suhar_) 10.04.2017		(end)
			break;
...
void CUIPdaWnd::Update()
{
	...
	luabind::functor<bool>	CanSwitchPdaMode;
	if ( ai().script_engine().functor("_g.can_run_script_pda",	CanSwitchPdaMode) )
	{
		if ( CanSwitchPdaMode() )
			can_switch = true;
	}
	m_btn_switch->Show(can_switch);
	// m_btn_switch->Show( Actor()->HasInfo("info_pda_advanced") );
	// Lex Addon (correct by Suhar_) 15.08.2018		(end)
}
