...
// Lex Addon (correct by Suhar_) 24.06.2016		(begin)
// Не помню, но зачем-то прикручивал эту библиотеку
#include <stdio.h>
...
bool CUIActorMenu::ToSlot(CUICellItem* itm, bool force_place, u16 slot_id)
{
	...
	{ // in case slot is busy
		// Lex Addon (correct by Suhar_) 21.01.2017		(begin)
		// Разрешаем перемешивать оружие в слотах
		if ( !force_place || slot_id == NO_ACTIVE_SLOT ) 
			return false;

		if ( m_pActorInvOwner->inventory().SlotIsPersistent(slot_id) && slot_id!=DETECTOR_SLOT && slot_id!=KNIFE_SLOT && slot_id!= BINOCULAR_SLOT  )
			return false;

		// Если пытаемся вставить в слот ножа и слот 2 пуст, то вставляем в слот 2
		if ( slot_id == KNIFE_SLOT && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_2) )
			return ToSlot(itm, force_place, INV_SLOT_2);
		// Если пытаемся вставить в слот ножа и слот 3 пуст, то вставляем в слот 3
		if ( slot_id == KNIFE_SLOT && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_3) )
			return ToSlot(itm, force_place, INV_SLOT_3);
		
		// Если пытаемся вставить в слот 2, слот ножа пуст и базовый слот объекта не =3, то вставляем в слот ножа
		if ( slot_id == INV_SLOT_2 && m_pActorInvOwner->inventory().CanPutInSlot(iitem, KNIFE_SLOT) && iitem->BaseSlot()!=INV_SLOT_3 )
			return ToSlot(itm, force_place, KNIFE_SLOT);
		// Если пытаемся вставить в слот 2 и слот 3 пуст, то вставляем в слот 3
		if ( slot_id == INV_SLOT_2 && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_3) )
			return ToSlot(itm, force_place, INV_SLOT_3);
		
		// Если пытаемся вставить в слот 3, слот ножа пуст и базовый слот объекта не =3, то вставляем в слот ножа
		if ( slot_id == INV_SLOT_3 && m_pActorInvOwner->inventory().CanPutInSlot(iitem, KNIFE_SLOT) && iitem->BaseSlot()!=INV_SLOT_3 )
			return ToSlot(itm, force_place, KNIFE_SLOT);
		// Если пытаемся вставить в слот 3 и слот 2 пуст, то вставляем в слот 2
		if ( slot_id == INV_SLOT_3 && m_pActorInvOwner->inventory().CanPutInSlot(iitem, INV_SLOT_2) )
			return ToSlot(itm, force_place, INV_SLOT_2);
		// Lex Addon (correct by Suhar_) 21.01.2017		(end)
...
// Lex Addon (correct by Suhar_) 24.06.2016		(begin)
// Функция переноса артефакта на пояс
bool CUIActorMenu::ToBelt(CUICellItem* itm, bool b_use_cursor_pos)
{
	// DEBUG
	// Log("fuction ToBelt has been called");
	// DEBUG

	PIItem	iitem						= (PIItem)itm->m_pData;
	bool b_own_item						= (iitem->parent_id()==m_pActorInvOwner->object_id());

	// Если на поясе есть свободные слоты
	if(m_pActorInvOwner->inventory().CanPutInBelt(iitem))
	{
		CUIDragDropListEx*	old_owner		= itm->OwnerList();
		CUIDragDropListEx*	new_owner		= NULL;
		// Перетаскиваем арт на пояс курсором
		if(b_use_cursor_pos)
			{
			new_owner						= CUIDragDropListEx::m_drag_item->BackList();
			VERIFY						(new_owner==m_pInventoryBeltList);
			}
		// Переносим арт на пояс двойным кликом из инвентаря
		else
	...
	// Если на поясе нет свободных слотов
	else
	{
		// Если предмет не предназначен для ношения на поясе или размер пояса = 0
		if(!iitem->Belt() || m_pActorInvOwner->inventory().BeltWidth()==0)
			// Тогда прерываем перенос
			return false;

		CUIDragDropListEx* belt_list		= NULL;
		if(b_use_cursor_pos)
			// Если вешаем артефакт вручную (перетаскиванием из рюкзака в слот курсором), то ...
			belt_list						= CUIDragDropListEx::m_drag_item->BackList();
		else
			// Если пытаемся повешать арт двойным нажатием на него в рюкзаке, то прерываем перенос
			return false;

		// Двумерный вектор координат слота, на который мы пытаемся перетищть арт
		Ivector2 belt_cell_pos				= belt_list->PickCell(GetUICursor().GetCursorPosition());
		// Если ткнули не на пояс, то коордианты вектора будут иметь значения (-1;-1) - прерываем перенос
		if(belt_cell_pos.x==-1 && belt_cell_pos.y==-1)
			return false;
		// Фильтры, защищающие от загрузки артефакта в закрытые слоты
		// Нахрен они нужны? Без них, при попытке переноса арта вручную на закрытый слот будем хавать вылет 
		// y - номер ряда слотов, колеблется в интервале [0;1] (всего на текущий момент 2 ряда)
		// x - позиция слота по горизонтали, колеблется в интервале [0;4] (всего 5 позиций на один ряд)
		// Если ткнули на первый ряд (y=0) и общее число доступных слотов оказалось меньше позиции x, то прерываем перенос
		if(belt_cell_pos.y==0 && (belt_cell_pos.x+1)>(m_pActorInvOwner->inventory().BeltWidth()))
			return false;
		// Если ткнули на второй ряд (y=1) и общее число доступных слотов оказалось меньше позиции x+6, то прерываем перенос
		/*
		Обязательно добавляем к позиции x 5 едениц на каждый ряд (начиная со второго),
		т.к. переменная слотов в костюме имеет тип скаляра и колеблется в интервале от [1;n],
		где n - это максимально возможное количество слотов под артефакты (сейчас - 10)
		*/
		if(belt_cell_pos.y==1 && (belt_cell_pos.x+6)>(m_pActorInvOwner->inventory().BeltWidth()))
			return false;
		
		// DEBUG
		// Вывод координат слотов в консоль (использовалось на этапе калибровки)
		// Все функции работают; при необходимости можно раскомментировать и отследить координаты через консоль
		// int bcpx = belt_cell_pos.x;
		// int bcpy = belt_cell_pos.y;
		// Log("bcpx = ", bcpx);
		// Log("bcpy = ", bcpy);
		// DEBUG
...
bool CUIActorMenu::ToQuickSlot(CUICellItem* itm)
{
	...
	// Lex Addon (correct by Suhar_) 3.09.2015		(begin)
	// edit 16.07.2016
	// Запрещаем толкать в ячейки быстрого доступа некоторые бустеры (дабы не ловить вылет при попытке вставки бустера с иконкой больше, чем 1х1)
	CObject*	pObj			= smart_cast<CObject*>		(iitem);
	shared_str	section_name	= pObj->cNameSect();
	// Читаем из секции параметр, разрешающий/запрещающий вставлять бустер в слот
	bool CanSwitchToFastSlot = READ_IF_EXISTS(pSettings, r_bool, section_name, "can_switch_to_fast_slot",    true );
	// Проверяем параметр (true - можно, false - нельзя)
	if (!CanSwitchToFastSlot)
		return false;
	// Lex Addon (correct by Suhar_) 3.09.2015		(end)
...
{
	//присоединение аддонов к активному слоту (2, 3 или слоту ножа)
	...
	// Lex Addon (correct by Suhar_) 28.09.2016		(begin)
	// Расширяем алгоритм навешивания обвесов на оружие
	PIItem	item_in_slot_2 = inv->ItemFromSlot(INV_SLOT_2);
	PIItem	item_in_slot_3 = inv->ItemFromSlot(INV_SLOT_3);
	// Переменная оружия в слоте ножа
	PIItem	item_in_knife_slot = inv->ItemFromSlot(KNIFE_SLOT);

	// Если все оружейные слоты пусты, то завершаем работу функции
	if(!item_in_slot_2 && !item_in_slot_3 && !item_in_knife_slot)	return;
...
//			m_UIPropertiesBox->AddItem( "st_attach_scope_to_rifle",  (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON );
		...
		if ( item_in_knife_slot && item_in_knife_slot->CanAttach(pScope) )
		{
			shared_str str = CStringTable().translate("st_attach_scope_to_pistol");
			str.printf("%s %s", str.c_str(), item_in_knife_slot->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_knife_slot, INVENTORY_ATTACH_ADDON );
//			m_UIPropertiesBox->AddItem( "st_attach_scope_to_rifle",  (void*)item_in_knife_slot, INVENTORY_ATTACH_ADDON );
			b_show			= true;
		}
		return;
	}

	// Присоединяем глушитель
	if ( pSilencer )
	{
...
//			m_UIPropertiesBox->AddItem( "st_attach_silencer_to_rifle",  (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON );
		...
		if ( item_in_knife_slot && item_in_knife_slot->CanAttach(pSilencer) )
		{
			shared_str str = CStringTable().translate("st_attach_silencer_to_pistol");
			str.printf("%s %s", str.c_str(), item_in_knife_slot->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_knife_slot, INVENTORY_ATTACH_ADDON );
//			m_UIPropertiesBox->AddItem( "st_attach_silencer_to_rifle",  (void*)item_in_knife_slot, INVENTORY_ATTACH_ADDON );
			b_show			= true;
		}
		return;
	}

	// Присоединяем подствольник
	if ( pGrenadeLauncher )
	{
...
//			m_UIPropertiesBox->AddItem( "st_attach_gl_to_rifle",  (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON );
		...
		if ( item_in_knife_slot && item_in_knife_slot->CanAttach(pGrenadeLauncher) )
		{
			shared_str str = CStringTable().translate("st_attach_gl_to_rifle");
			str.printf("%s %s", str.c_str(), item_in_knife_slot->m_name.c_str());
			m_UIPropertiesBox->AddItem( str.c_str(),  (void*)item_in_knife_slot, INVENTORY_ATTACH_ADDON );
//			m_UIPropertiesBox->AddItem( "st_attach_gl_to_rifle",  (void*)item_in_knife_slot, INVENTORY_ATTACH_ADDON );
			b_show			= true;
		}
	// Lex Addon (correct by Suhar_) 28.09.2016		(end)
	}
