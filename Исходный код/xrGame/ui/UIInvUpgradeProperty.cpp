...
UIProperty::UIProperty()
{
	...
	// Добавлена возможность опционально включать отображение апгрейда в описании предмета
	ShowStatus = NULL;
...
bool UIProperty::init_property( shared_str const& property_id )
{
	...
	// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
	// Получаем статус показа данного свойства в описании предметов
	ShowStatus = get_property()->m_show_status;
	// Lex Addon (correct by Suhar_) 5.08.2018		(end)
	return true;
}
...
// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
// Дополнительный аргумент bool item_info; показывает, где будет ли это свойство отображаться в описании предмета
bool UIProperty::compute_value( ItemUpgrades_type const& item_upgrades, bool item_info )
// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
{
	if ( !get_property() )
	{
	...
	// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
	// Добавлена возможность опционально включать отображение апгрейда в описании предмета
	// Если item_info==true, значит свойство отображается в описании предмета а не апгрейда у техника
	// Если ShowStatus==false, значит для данного свойства отображение в описании предмета отключено
	// Запрещаем показ, если оба условия выполнены
	if ( item_info && !ShowStatus )
	{
		return false;
	}
	// Lex Addon (correct by Suhar_) 5.08.2018		(end)
...
// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
// Дополнительный аргумент bool item_info; показывает, где будет ли это свойство отображаться в описании предмета
void UIInvUpgPropertiesWnd::set_info( ItemUpgrades_type const& item_upgrades, bool item_info )
{
	Fvector2 new_size;
	...
	for ( ; ib != ie ; ++ib )
	{
		...
		if ( ui_property->compute_value( item_upgrades, item_info ) )
		// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
		{
...
void UIInvUpgPropertiesWnd::set_upgrade_info( Upgrade_type& upgrade )
{
	...
	// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
	// Дополнительный аргумент типа bool для функции set_info
	// Показывает, где будет ли это свойство отображаться в описании предмета
	// В данном случае свойства отображаются в окне описания улучшения
	set_info( m_temp_upgrade_vector, false );
	// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
}
...
{
	// Отображаем свойства у предмета в инвентаре
	set_info( item.upgardes(), true );
}
// Lex Addon (correct by Suhar_) 5.08.2018		(begin)
