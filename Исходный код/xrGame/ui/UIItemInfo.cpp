...
void CUIItemInfo::InitItemInfo(LPCSTR xml_name)
{
	...
	// Lex Addon (correct by Suhar_) 1.08.2018		(begin)
	// Добавляем статик количества улучшений на экран
	if(uiXml.NavigateToNode("static_upgrades_count",0))
	{
		UIUpgradesCount				= xr_new<CUITextWnd>();	 
		AttachChild					(UIUpgradesCount);		
		UIUpgradesCount->SetAutoDelete(true);
		xml_init.InitTextWnd		(uiXml, "static_upgrades_count", 0,	UIUpgradesCount);
	}
	// Lex Addon (correct by Suhar_) 1.08.2018		(end)
...
void CUIItemInfo::InitItem(CUICellItem* pCellItem, CInventoryItem* pCompareItem, u32 item_price, LPCSTR trade_tip)
{
	...
	if ( UIWeight )
	{
		...
		// Lex Addon (correct by Suhar_) 27.09.2016		(begin)
		// Здесь можно изменить количество разрядов после запятой, отображаемых в графе массы объекта
		xr_sprintf				(str, "%.2f %s", weight, kg_str );
		// Lex Addon (correct by Suhar_) 27.09.2016		(end)
	...
	// Lex Addon (correct by Suhar_) 1.08.2018		(begin)
	// Добавляем статик количества улучшений на экран
	int	UpgradesCount = pInvItem->upgardes().size();
	if (UIUpgradesCount && IsGameTypeSingle() && item_price==u32(-1) && UpgradesCount>=1)
	{
		xr_sprintf(str, "%s %d", CStringTable().translate("ui_upgrades_count").c_str(), UpgradesCount);
		UIUpgradesCount->SetText(str);
		pos.x = UIUpgradesCount->GetWndPos().x;
		if (m_complex_desc)
		{
			UIUpgradesCount->SetWndPos(pos);
		}
		UIUpgradesCount->Show(true);
	}
	else
		UIUpgradesCount->Show(false);
	// Lex Addon (correct by Suhar_) 1.08.2018		(end)
