...
{
	// Lex Addon (correct by Suhar_) 28.09.2016		(begin)
	// Подсветка слота ножа
	m_InvSlotKnifeHighlight->Show(false);
	// Подсветка слота бинокля
	m_InvSlotBinocularHighlight->Show(false);
	// Lex Addon (correct by Suhar_) 28.09.2016		(end)
...
// Lex Addon (correct by Suhar_) 28.09.2016		(begin)
// Функция подсветки слотов выделенного предмета
void CUIActorMenu::highlight_item_slot(CUICellItem* cell_item)
{
	PIItem item = (PIItem)cell_item->m_pData;
	if(!item)
		return;
	// Получаем базовый слот предмета
	...
	// Если выбранный объект - оружие
	if(weapon)
	{
		// Если базовый слот предмета - слот биноклей
		if(item_slot==BINOCULAR_SLOT)
		{
			// Подсвечиваем слот бинокля
			m_InvSlotBinocularHighlight->Show(true);
			return;
		}
		// Если базовый слот предмета - слот для ножей или для пистолетов
		if(item_slot==KNIFE_SLOT || item_slot==INV_SLOT_2)
		{
			// Подсвечиваем все оружейные слоты
			m_InvSlotKnifeHighlight->Show(true);
			m_InvSlot2Highlight->Show(true);
			m_InvSlot3Highlight->Show(true);
			return;
		}
		// Если базовый слот предмета - слот для винтовок
		if(item_slot==INV_SLOT_3)
		{
			// Подсвечиваем слоты пистолетов и винтовок
			m_InvSlot2Highlight->Show(true);
			m_InvSlot3Highlight->Show(true);
			return;
		}
...
// Lex Addon (correct by Suhar_) 28.09.2016		(end)
	...
	{
		// 16.07.2016
		// Запрещаем подсвечивать ячейки быстрого доступа при наведении курсора на бустеры которые невозможно вставить в быстрые слоты
	...
	// Разрешаем подсветку пояса для предметов, которые можно повесить на пояс
	bool CanSwitchToBelt = READ_IF_EXISTS(pSettings, r_bool, section_name, "belt",    false );
	...
	// Lex Addon (correct by Suhar_) 11.03.2017		(end)
	{
		...
		// Подсвечиваем доступные слоты на поясе при наведении курсора на артефакт в инвентаре
		// Потолок i вычисляется как произведение количества строк на количество слотов в строке
...
void CUIActorMenu::UpdateActorMP()
{
	...
	// Lex Addon (correct by Suhar_) 4.09.2015		(begin)
	// Меняем иконку в сетевой игре
	m_ActorCharacterInfo->InitCharacterMP( Game().local_player->getName(), "ui_no_portret" );
	// Lex Addon (correct by Suhar_) 4.09.2015		(end)
...
bool CUIActorMenu::CanSetItemToList(PIItem item, CUIDragDropListEx* l, u16& ret_slot)
{
	...
	// Lex Addon (correct by Suhar_) 28.09.2016		(begin)
	// Разрешаем вставлять в слот ножа пистолеты
	if(item_slot==INV_SLOT_2 && l==m_pInventoryKnifeList)
	{
		ret_slot	= KNIFE_SLOT;
		return		true;
	}

	// Разрешаем вставлять в слот винтовок ножи и пистолеты
	if((item_slot==KNIFE_SLOT && l==m_pInventoryAutomaticList)||(item_slot==INV_SLOT_2 && l==m_pInventoryAutomaticList))
	{
		ret_slot	= INV_SLOT_3;
		return		true;
	}

	// Разрешаем вставлять в слот пистолетов ножи и винтовки
	if((item_slot==KNIFE_SLOT && l==m_pInventoryPistolList)||(item_slot==INV_SLOT_3 && l==m_pInventoryPistolList))
	{
		ret_slot	= INV_SLOT_2;
		return		true;
	}
...
{
	// Lex Addon (correct by Suhar_) 28.09.2016		(begin)
	// Отображаем индикатор состояния оружия в слоте ножа
	PIItem itm = m_pActorInvOwner->inventory().ItemFromSlot(KNIFE_SLOT);
	if(itm)
	{
