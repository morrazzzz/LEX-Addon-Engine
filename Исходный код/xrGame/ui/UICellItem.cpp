...
// Апдейт линейки состояния предмета в инвентаре
void CUICellItem::UpdateConditionProgressBar()
{
	if(m_pParentList && m_pParentList->GetConditionProgBarVisibility())
	{
		...
		// Линейка отображаетмя только у оружия, бронежилетов и шлемов
		if(pWeapon || pOutfit || pHelmet)
		{
			// Проверяем, не относится ли данный предмет к оружию, у которого показывать линейку состояния не нужно
			if(pWeapon)
			{
				// Смотрим базовый слот предмета
				u16 item_slot = itm->BaseSlot();
				// Предмет не должен относиться к ножам и биноклям
				if(item_slot==KNIFE_SLOT || item_slot==BINOCULAR_SLOT)
				{
					// Не показываем линейку состояния
