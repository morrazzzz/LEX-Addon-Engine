...
void CWeaponShotEffector::Shot( CWeapon* weapon )
{
	...
	// Учитываем множитель отдачи глушителя и патрона
	float angle	= m_cam_recoil.Dispersion    * weapon->cur_silencer_koef.cam_dispersion * weapon->m_magazine.back().param_s.kAngle;
	angle      += m_cam_recoil.DispersionInc * weapon->cur_silencer_koef.cam_disper_inc * weapon->m_magazine.back().param_s.kAngle * (float)m_shot_numer;
