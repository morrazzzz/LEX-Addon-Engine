...
// Lex Addon (correct by Suhar_) 8.10.2018		(begin)
// Добавлен метод для установки апгрейдов на предмет (по клиенскому объекту)
void CScriptGameObject::AddUpgrade(LPCSTR upgrade_id)
{
	CInventoryItem *inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item)
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member AddUpgrade!");
		return;
	}
	// Можем установить апгрейд только при условии, что этот апгрейд еще не установлен
	if (!inventory_item->has_upgrade(upgrade_id))
		inventory_item->net_Spawn_install_upgrade(upgrade_id);
}

// Добавлен метод для запроса, установлен ли у объекта апгрейд (по клиенскому объекту)
bool CScriptGameObject::UpgradeExist(LPCSTR upgrade_id)
{
	CInventoryItem *inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item)
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CSciptEntity : cannot access class member UpgradeExist!");
		return false;
	}
	return inventory_item->has_upgrade(upgrade_id);
}
// Lex Addon (correct by Suhar_) 8.10.2018		(end)

// Lex Addon (correct by Suhar_) 18.11.2018		(begin)
// Дополнительные методы для работы с аддонами
// Отсоединить прицел
...
// Получить название установленного подствольника
...
// Получить название установленного глушителя
...
// Присоединить аддон
void CScriptGameObject::attach_addon(u16 ItemID)
{
	CInventoryItem* addon = smart_cast<CInventoryItem*>(Level().Objects.net_Find(ItemID));
	CInventoryItem *inventory_item = smart_cast<CInventoryItem*>(&object());
	if (addon && inventory_item)
	{
		CWeapon* weapon = smart_cast<CWeapon*>(inventory_item);
		if (weapon)
			weapon->Attach(addon, true);
	}
}
// Lex Addon (correct by Suhar_) 18.11.2018		(end)
