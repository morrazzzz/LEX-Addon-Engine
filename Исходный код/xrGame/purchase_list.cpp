...
void CPurchaseList::process	(const CGameObject &owner, const shared_str &name, const u32 &count, const float &probability)
{
	...
	// Корректировка генерации случайных величин при обновлении ассортимента у торговца
	// Конструкция CPU::QPC() при последовательном вызове метода process в течении очень короткого промежутка времени
	// выдаёт значения, которые мало отличаются от предыдущих;
	...
	for (u32 i=0, j=0; i<count; ++i) {
		// В качестве источника энтропии будем использовать другой способ (разница в регистре!!!)
