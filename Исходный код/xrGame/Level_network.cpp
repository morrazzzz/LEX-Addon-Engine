...
{
	// Lex Addon (correct by Suhar_) 23.05.2018		(begin)
	// Клиентские объекты будут синхронизироваться с серверным чётко при сохранении (предложил Shoker)
	if (GameID() != eGameIDSingle || OnClient())
	// Lex Addon (correct by Suhar_) 23.05.2018		(end)
	{
