...
public:
	...
	// Lex Addon (correct by Suhar_) 2.11.2018		(begin)
	// Правка направления света фонарика в режиме от третьего лица
	IC EActorCameras		active_cam			()	{return cam_active;}
	// Lex Addon (correct by Suhar_) 2.11.2018		(end)
...
protected:
	...
	float					m_fWalkAccel;
	// Базовая переменная скорости передвижения (читается из конфига актора)
	float					m_fBaseWalkAccel;
	// Переменная предела скорости передвижения (читается из конфига актора)
	float					m_fWalkAccelLimit;
	// Переменная скорости прыжка
	float					m_fJumpSpeed;
	// Базовая переменная скорости прыжка (читается из конфига актора)
	float					m_fBaseJumpSpeed;
	// Переменная предела скорости прыжка (читается из конфига актора)
