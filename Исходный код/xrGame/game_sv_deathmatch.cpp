...
{
	// Lex Addon (correct by Suhar_) 1.09.2015		(begin)
	// Комментируем следующую строку, чтобы при респауне игрок ВСЕГДА получал чистый стартовый бонус (в оригинале - только если начинаешь без снаряги)
	//if (!ps) return;
	if (m_bInWarmUp) return;
	//if (ps->LastBuyAcount != 0) return;
	// Lex Addon (correct by Suhar_) 1.09.2015		(end)
