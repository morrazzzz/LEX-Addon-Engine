...
void CWeapon::UpdateCL		()
{
	...
	// Lex Addon (correct by Suhar_) 23.04.2017		(begin)
	// UPD: 24.10.2018
	// Добавляем возможность отключать ПНВ, встроенный в прицел, нажатием забинденной клавиши
	if(m_zoom_params.m_pNight_vision && !need_renderable())
	{
		...
		// Если ПНВ прицела выключен и есть фонарь
		if (pTorch && !m_zoom_params.m_pNight_vision->IsActive())
		{
			// Если до прицеливания был включен ПНВ на костюме или шлеме, то необходимо это запомнить
			...
			// Если флаг активации ПНВ, встроенного в прицел, говорит о включении, то
			if (pTorch->EnableWeaponNightVision)
				// Включаем встроенный в прицел ПНВ
				m_zoom_params.m_pNight_vision->Start(m_zoom_params.m_sUseZoomPostprocess, pA, false);
		}
		// Если ПНВ прицела включен, есть фонарь и
		// флаг активации ПНВ, встроенного в прицел, говорит о выключении, то
		else if(pTorch && !pTorch->EnableWeaponNightVision)
		{
			// Выключаем встроенный в прицел ПНВ
			m_zoom_params.m_pNight_vision->Stop(10.0f, false);
		}
	}
	else
	{
	...
	// Lex Addon (correct by Suhar_) 23.04.2017		(end)
...
// Корректировка работы прицелов
// Здесь будет храниться последнее значение зума для регулируемых прицелов
float LastZoomFactor = NULL;
void CWeapon::OnZoomIn()
{
	m_zoom_params.m_bIsZoomModeNow		= true;
	// Если доступен регулируемый зум и установлен прицел
	if (m_zoom_params.m_bUseDynamicZoom && IsScopeAttached())
	{
		// По возможности, используем сохраненное значение зума, чтобы не настраивать кратность заново каждый раз при прицеливании
		if (LastZoomFactor)
		...
		// Контролируем, чтобы значение зума оставалось в допустимом интервале
...
void CWeapon::ZoomInc()
{
	...
	SetZoomFactor			( f );
	// Lex Addon (correct by Suhar_) 24.10.2018		(begin)
	// Запоминаем текущее значение зума, чтобы не настраивать кратность заново каждый раз при прицеливании
	LastZoomFactor			= f;
	// Lex Addon (correct by Suhar_) 24.10.2018		(end)
}
