...
struct SCartridgeParam
{
	...
	// Добавлен множитель начальной скорости (kSpeed)
	// Добавлен множитель отдачи (kAngle)
	float	kDist, kDisp, kHit/*, kCritical*/, kImpulse, kAP, kAirRes, kSpeed, kAngle;
	int		buckShot;
	float	impair;
	float	fWallmarkSize;
	u8		u8ColorID;
	...
	IC void Init()
	{
		kDist = kDisp = kHit = kImpulse = kSpeed = kAngle = 1.0f;
