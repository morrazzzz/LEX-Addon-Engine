...
// Lex Addon (correct by Suhar_) 24.02.2017		(begin)
// Задаём возможность управлять fov из конфига актора
float g_fov = 55.0f;
bool use_opt_fov = true;
// Lex Addon (correct by Suhar_) 24.02.2017		(end)
...
void CActor::Load	(LPCSTR section )
{
	...
	*/
	...
	// Задаём переменную базовой скорости передвижения - читаем из конфига
	m_fBaseWalkAccel			= pSettings->r_float(section,"walk_accel");
	// Задаём предел скорости передвижения - читаем из конфига
	m_fWalkAccelLimit			= pSettings->r_float(section,"walk_accel_limit");
	// Задаём значение переменной скорости передвижения - читаем базовую переменную
	m_fWalkAccel				= m_fBaseWalkAccel;
	// Задаём переменную базовой скорости прыжка - читаем из конфига
	m_fBaseJumpSpeed			= pSettings->r_float(section,"jump_speed");
	// Задаём предел скорости прыжка - читаем из конфига
	m_fJumpSpeedLimit			= pSettings->r_float(section,"jump_speed_limit");
	// Задаём значение переменной скорости прыжка - читаем базовую переменную
	m_fJumpSpeed				= m_fBaseJumpSpeed;
	// Lex Addon (correct by Suhar_) 6.07.2016		(end)
	// Lex Addon (correct by Suhar_) 24.02.2017		(begin)
	// Задаём возможность управлять fov из конфига актора (если прописано)
	// Если не прописано - управлять fov из меню настроек 17.09.2018
	if (pSettings->line_exist(section, "base_fov")) 
	{
		g_fov					= pSettings->r_float(section, "base_fov");
		use_opt_fov				= false;
	}
	else
		g_fov					= m_fBaseFov;
	// Lex Addon (correct by Suhar_) 24.02.2017		(end)
	// Lex Addon (correct by Suhar_) 25.05.2018		(begin)
	// Задаём дефолтное значение здоровья актора
	ActorStartHealth			= pSettings->r_float(section,"Health")/100;
	// Lex Addon (correct by Suhar_) 25.05.2018		(end)
...
float CActor::currentFOV()
{
	...
	// Lex Addon (correct by Suhar_) 22.06.2018		(begin)
	// Правка позиции ведения огня для режима камеры от третьего лица
	// Частично заимствовано: X-Ray Oxygen
	...
	// Lex Addon (correct by Suhar_) 22.06.2018		(end)
}
...
void CActor::UpdateCL	()
{
	...
	// Lex Addon (correct by Suhar_) 17.09.2018		(begin)
	// Задаём возможность управлять fov из меню настроек
	if (use_opt_fov)
		g_fov = m_fBaseFov;
	// Lex Addon (correct by Suhar_) 17.09.2018		(end)
}
...
void CActor::g_PerformDrop	( )
{
	...
	// Разрешаем выбрасывать предметы из слотов ножа и бинокля
	if(inventory().SlotIsPersistent(s) && s!=KNIFE_SLOT && s!= BINOCULAR_SLOT) return;
...
void CActor::UpdateArtefactsOnBeltAndOutfit()
{
	...
	// Переменная для подсёта дополнительного здоровья из свойств артефактов и костюма 4.07.2016
	float max_health_add = 0.00;
	// Переменная для подсчёта дополнительной скорости прыжка из свойств артефактов и костюма 6.07.2016
	float jump_speed_add = 0.00;
	// Переменная для подсчёта дополнительной скорости передвижения из свойств артефактов и костюма
		...
		inventory().m_belt.end() != it; ++it) 
	{
		...
		if(artefact)
		{
			...
			// Lex Addon (correct by Suhar_)				(begin)
			// Подсчитываем значение дополнительного здоровья из свойств артефактов 4.07.2016
			max_health_add	+=				(artefact->m_fMaxHealthAdd);
			// Подсчитываем значение дополнительной скорости прыжка из свойств артефактов 6.07.2016
			jump_speed_add	+=				(artefact->m_fJumpSpeedAdd);
			// Подсчитываем значение дополнительной скорости передвижения из свойств артефактов
			walk_accel_add	+=				(artefact->m_fWalkAccelAdd);
			// Lex Addon (correct by Suhar_)				(end)
	...
	if ( outfit )
	{
		...
		// Lex Addon (correct by Suhar_) 				(begin)
		// Подсчитываем значение дополнительного здоровья из свойств костюма 4.07.2016
		max_health_add	+=				(outfit->m_fMaxHealthAdd);
		// Подсчитываем значение дополнительной скорости прыжка из свойств костюма 6.07.2016
		jump_speed_add	+=				(outfit->m_fJumpSpeedAdd);
		// Подсчитываем значение дополнительной скорости передвижения из свойств костюма
		walk_accel_add	+=				(outfit->m_fWalkAccelAdd);
		// Lex Addon (correct by Suhar_) 				(end)
	}
	else
	{
	...
	// Lex Addon (correct by Suhar_) 				(begin)
	// 4.07.2016
	// Если результирующее значение максимального здоровья меньше или равно 0, то
	if (ActorStartHealth + max_health_add <= 0)
		// Устанавливаем максимальное значение здоровья на 0.01 (дабы избежать смерти ГГ или неприятностей с двжиком, ибо хз что будет, если не фильтровать)
		conditions().max_health() = 0.01;
	// Иначе
	else
		// Присваиваем переменной здоровья дефолтное значение + бонусы от артов
		conditions().max_health() = ActorStartHealth + max_health_add;
		// conditions().max_health() = m_entity_condition->max_health() + max_health_add;
	// 6.07.2016
	// Если результирующее значение скорости прыжка меньше или равно 0, то
	if (m_fBaseJumpSpeed + jump_speed_add <=0)
		// Устанавливаем на 0.01 (опять же хз, что будет, если не фильтровать)
		m_fJumpSpeed = 0.01;
	// Иначе если результирующее значение скорости прыжка превысило допустимый предел, то
	else if (m_fBaseJumpSpeed + jump_speed_add > m_fJumpSpeedLimit)
		// Устанавливаем на скорость прыжка на допустимый предел
		m_fJumpSpeed = m_fJumpSpeedLimit;
	// Иначе
	else
		// Присваиваем переменной скорости прыжка базовое значение + бонусы от артов
		m_fJumpSpeed = m_fBaseJumpSpeed + jump_speed_add;
	// Заносим полчившееся значение в менеджер движения
	character_physics_support()->movement()->SetJumpUpVelocity(m_fJumpSpeed);
	// Меняем значение переменной скорости передвижения, исходя из добавочного значения
	...
	// DEBUG
	// Вывод значения максимального и текущего здоровья ГГ в консоль (использовалось на этапе внедрения)
	// float maxhealth = conditions().max_health();
	// float hp = conditions().health();
...
float CActor::GetRestoreSpeed( ALife::EConditionRestoreType const& type )
{
	...
	case ALife::ePowerRestoreSpeed:
	{
		...
		// Lex Addon (correct by Suhar_) 				(begin)
		// 31.08.2015
		// Изменённый алгоритм рассчёта восстановления сил
		// Считываем параметр PowerLoss и PowerRestoreSpeed теперь и у шлемов (в оригинале считывали только у костюмов)
		// Переменная костюма в слоте
		CCustomOutfit* outfit = GetOutfit();
		// Переменная шлема в слоте
		CHelmet* helmet = smart_cast<CHelmet*>(inventory().ItemFromSlot(HELMET_SLOT));
		// Если есть костюм, но нет шлема
		if (outfit && !helmet)
		{
		...
		else if (outfit && helmet)
		{
			...
			VERIFY(outfit->m_fPowerLoss!=0.0f);
			// 13.07.2016
			// Увеличена скорость падения выносливости
			res /= (helmet->m_fPowerLoss)+(outfit->m_fPowerLoss)-1.0f;
		}
		// Если в слотах брони нет ничего
		else
			res /= 1.0f;
		break;
		// Lex Addon (correct by Suhar_) 				(end)
