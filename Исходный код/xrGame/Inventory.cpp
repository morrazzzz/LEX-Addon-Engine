...
float CInventory::CalcTotalWeight()
{
	...
	for(TIItemContainer::const_iterator it = m_all.begin(); m_all.end() != it; ++it) 
		weight += (*it)->Weight();
	// Lex Addon (correct by Suhar_) 29.11.2018		(begin)
	// Предотвращаем отрицательное значение общей массы предметов в инвентаре
	if (weight<0)
		weight = 0;
	// Lex Addon (correct by Suhar_) 29.11.2018		(end)
