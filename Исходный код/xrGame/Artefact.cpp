...
void CArtefact::Load(LPCSTR section) 
{
	...
	// Добавляем чтение дополнительного здоровья из свойств артефакта 4.07.2016
	m_fMaxHealthAdd			 = pSettings->r_float	(section,"max_health_add"			);
	// Добавляем чтение дополнительной скорости прыжка из свойств артефакта 6.07.2016
	m_fJumpSpeedAdd			 = pSettings->r_float	(section,"jump_speed_add"			);
	// Добавляем чтение дополнительной скорости передвижения из свойств артефакта
	m_fWalkAccelAdd			 = pSettings->r_float	(section,"walk_accel_add"			);
