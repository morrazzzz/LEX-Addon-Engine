...
void CAI_Stalker::Hit(SHit* pHDS)
{
	...
	{
		// Lex Addon (correct by Suhar_) 16.07.2016		(begin)
		// Читаем параметр брони кости
		float BoneArmor = m_boneHitProtection->getBoneArmor(HDS.bone());
		// Читаем параметр защиты кости
		float BoneProtection = m_boneHitProtection->getBoneProtection(HDS.bone());
		...
		if(!fis_zero(BoneArmor, EPS))
		{
			if(ap > BoneArmor)
			{
				...
				// В случае пробития учитываем параметр защиты каждой кости
				// Чтобы можно было наносить дополнительный хит при попадании в уязвимую часть тела (голову, например)
				hit_power /= BoneProtection;
				// Lex Addon (correct by Suhar_) 16.07.2016		(end)
