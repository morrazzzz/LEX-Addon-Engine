...
bool CWeaponMagazined::GetBriefInfo( II_BriefInfo& info )
{
	...
	else
	{
		...
		// Расширен алгоритм отображения количества патронов в правой панели.
		...
		// Переменная количества патронов, которые не заряжены, но поддерживаются данным оружием
		int add_ammo_count = 0;
		// Проверяем количество патронов для каждого поддерживаемого типа
		for (int i=0; i<at_size; i++)
		{
			// Если заряжен данный тип, то
			if (m_ammoType==i)
			{
				// Считываем количество патронов по данному типу и выводим его в левый счётчик
			...
			{
				// Учитываем число патронов данного типа
				add_ammo_count		+= GetAmmoCount(i);
			}
		}
		// Если оружие поддерживает несколько типов патронов (1 и более), то
		if ( at_size > 1 )
			// Приводим переменную количества к строковому типу
			xr_sprintf( int_str, "%d", add_ammo_count );
		// Иначе
		else
			// Дополнительные типы не поддерживаются; оставляем поле пустым
			xr_sprintf( int_str, "%s", "" );
		// Записываем строку количества дополнительных патронов в правый счётчик
