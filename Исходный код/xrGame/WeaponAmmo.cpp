...
//.	param_s.kCritical			= pSettings->r_float(section, "k_hit_critical");
	...
	param_s.kSpeed				= pSettings->r_float(section, "k_speed");
	// Считываем множитель отдачи
	param_s.kAngle				= pSettings->r_float(section, "k_angle");
...
//.	cartridge_param.kCritical	= pSettings->r_float(section, "k_hit_critical");
	...
	// Lex Addon (correct by Suhar_) 25.02.2017		(begin)
	// Считываем множитель начальной скорости
	cartridge_param.kSpeed		= pSettings->r_float(section, "k_speed");
	// Считываем множитель отдачи
	cartridge_param.kAngle		= pSettings->r_float(section, "k_angle");
	// Lex Addon (correct by Suhar_) 25.02.2017		(end)
