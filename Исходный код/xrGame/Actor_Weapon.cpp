...
{
	// Lex Addon (correct by Suhar_) 17.06.2018		(begin)
	...
	if (weapon)
	{
		// Стреляем от первого лица или с прицела
		if (eacFirstEye == cam_active || (weapon->IsZoomed() && weapon->ZoomTexture() && !weapon->IsRotatingToZoom()))
		{
			fire_dir = Cameras().Direction();
			fire_pos = Cameras().Position();
		}
		// Стреляем от третьего лица, не прицеливаясь
		else
		{
	...
	else
	{
		...
		if (pMissile)
		{
			...
			// От третьего лица
				...
				fire_dir = Level().CurrentControlEntity()->Direction();
		}
	}
	// Lex Addon (correct by Suhar_) 17.06.2018		(end)
}
