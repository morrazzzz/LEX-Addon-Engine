...
// Lex Addon (correct by Suhar_) 14.08.2018		(begin)
// Запуск движкового или скриптового интерфейса КПК
// Вспомогательные функции для работы со скриптами
struct SLuaPdaParams
{
	luabind::functor<bool>	NeedRunScriptPda;
	luabind::functor<void>	RunScriptPda;
	
	bool functor_1_exist;
	bool functor_2_exist;
	
	SLuaPdaParams();
	~SLuaPdaParams();
};
SLuaPdaParams::SLuaPdaParams()
{
	functor_1_exist	= ai().script_engine().functor("_g.need_run_script_pda",	NeedRunScriptPda);
	functor_2_exist	= ai().script_engine().functor("_g.run_script_pda",			RunScriptPda);
}
SLuaPdaParams::~SLuaPdaParams()
{
}
SLuaPdaParams* g_lua_pda_params = NULL;
void destroy_lua_pda_params()
{
	if(g_lua_pda_params)
		xr_delete(g_lua_pda_params);
}
// Lex Addon (correct by Suhar_) 14.08.2018		(end)
...
bool CUIGameSP::IR_UIOnKeyboardPress(int dik) 
{
	...
	if ( !g_lua_pda_params )
	{
		g_lua_pda_params = xr_new<SLuaPdaParams>();
	}
	// Lex Addon (correct by Suhar_) 14.08.2018		(end)
	...
	case kACTIVE_JOBS:
		{
			...
			if ( !pActor->inventory_disabled() )
				// Проверка по наличию инфопоршна (так это работало в ранних версиях аддона)
				// if ( Actor()->HasInfo("show_script_pda_info") )
				// Так это работает теперь
				// Если в скриптах прописаны все необходимые функции
				if (g_lua_pda_params->functor_1_exist && g_lua_pda_params->functor_2_exist)
				{
					// Проверяем необходимо ли запускать скриптовый интерфейс
					if (g_lua_pda_params->NeedRunScriptPda())
						// Если необходимо - запускаем
						g_lua_pda_params->RunScriptPda();
					// Если нет, то запускаем движковый
					else
						ShowPdaMenu();
				}
				// Нет необходимых функций. Отображаем движковый интерфейс.
				else
					ShowPdaMenu();
			break;
			// Lex Addon (correct by Suhar_) 14.08.2018		(end)
