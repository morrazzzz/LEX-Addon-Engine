...
void Property::construct( shared_str const& property_id, Manager& manager_r )
{
	...
	// Читаем из конфига статус показа данного свойства в описании предметов
	// Если данного параметра в секции нет, то будем считать, что показ необходим
	m_show_status = READ_IF_EXISTS( pSettings, r_bool, id(), "attach_to_describe", true );
