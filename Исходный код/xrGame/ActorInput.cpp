...
void CActor::SwitchNightVision()
{
	...
	// Добавляем возможность включать/выключать встроенный в прицел ПНВ предмету в слоте ножа
	CWeapon* wpn1 = NULL;
	CWeapon* wpn2 = NULL;
	CWeapon* wpn3 = NULL;
	// Добавляем возможность включать/выключать встроенный в прицел ПНВ предмету в слоте бинокля
	CWeapon* binoc = NULL;
	if(inventory().ItemFromSlot(KNIFE_SLOT))
		wpn1 = smart_cast<CWeapon*>(inventory().ItemFromSlot(KNIFE_SLOT));
	...
	if(inventory().ItemFromSlot(BINOCULAR_SLOT))
		binoc = smart_cast<CWeapon*>(inventory().ItemFromSlot(BINOCULAR_SLOT));
	...
	for ( ; it != it_e; ++it )
	{
		...
		if ( torch )
		{
			// Отслеживаем нажатие на клавишу активации ПНВ при прицеливании;
			// если находимся в состоянии прицеливания, то меняем флаг активации ПНВ, встроенного в прицел, на противоположный
			if(wpn1 && wpn1->IsZoomed())
			{
				torch->EnableWeaponNightVision = !torch->EnableWeaponNightVision;
				return;
			}
