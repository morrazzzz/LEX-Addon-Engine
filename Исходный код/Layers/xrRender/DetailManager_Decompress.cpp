...
void		CDetailManager::cache_Decompress(Slot* S)
{
	...
	float		density		= ps_r__Detail_density * 10;
	// Lex Addon (correct by Suhar_) 16.09.2018		(begin)
	// Изменяем пределы плотности травы; калибровка параметров с учетом работы старого алгоритма (дабы не нарушить правильную работу)
	// float		jitter		= density/1.7f;
	// u32			d_size		= iCeil	(dm_slot_size/density);
	float jitter = 0.59f;
	if (density>=2)
		jitter = 1.17f / density;
	if (jitter>0.59f)
		jitter = 0.59f;
	u32			d_size		= iFloor(density);
	// Lex Addon (correct by Suhar_) 16.09.2018		(end)
